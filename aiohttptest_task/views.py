...
async def news_list(request):
    ...
    for item in data_news:
        time_item = datetime.strptime(item.get('date'), '%Y-%m-%dT%H:%M:%S')
        # Not deleted and valid time
...
async def add_new_news(request):
    ...
    return web.HTTPFound(location=router['results'])


async def delete_news_by_id(request):
    news_id = request.match_info['news_id']

    async with aiofiles.open(DATA_PATH+DATA_NEWS, mode='r') as json_file:
    ...
    router = request.app.router
    url = router['results'].url_for(news_id=str(news_id))
